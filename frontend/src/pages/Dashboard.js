  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setEssays(essRes.data.essays || essRes.data);
    ...
    // Слушатель события обновления истории - только для Listening
    const handleListeningHistoryUpdated = () => {
      if (!isMounted) return;
      
      // Обновляем только listening данные, не все
      api.get('/listening/sessions/')
        .then(res => {
          if (isMounted) {
        ...
        .catch(err => console.error('Ошибка при обновлении listening:', err));
    };
    ...
    <div className="p-3 sm:p-4 md:p-8 lg:p-10 max-w-full md:max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-8">
        ...
        <button
          ...
          className="bg-blue-600 text-white rounded-xl px-3 sm:px-4 py-2 text-sm sm:text-base hover:bg-blue-700 w-full mt-2 sm:mt-0"
        >
          Start New Test
        </button>
      </div>
          ...
          <div key={idx} className="bg-white rounded-xl shadow border p-3 flex flex-col gap-2">
            ...
            <div className="flex justify-between items-center mt-1">
              ...
              {item.type === 'Reading' && item.item.id && (
                <button onClick={() => navigate(`/reading-result/${item.item.id}`)} className="text-indigo-600 hover:text-indigo-900 text-xs font-semibold">Details</button>
              )}
              {item.type === 'Listening' && item.item.id && (
                <button onClick={() => navigate(`/listening-result/${item.item.id}`)} className="text-indigo-600 hover:text-indigo-900 text-xs font-semibold">Details</button>
              )}
              {item.type === 'Writing' && (
      ...
      {selectedItem && selectedItem.type === 'Reading' && (
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-xl p-3 sm:p-6 max-w-full sm:max-w-2xl md:max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg sm:text-xl font-bold">Reading Test Results — {selectedItem.item.submitted_at?.slice(0, 10)}</h3>
              <button onClick={handleCloseDetails} className="text-red-600 hover:underline">Close</button>
            </div>
            {detailsLoading ? (
      ...
      {selectedItem && selectedItem.type === 'Listening' && (
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-xl p-3 sm:p-6 max-w-full sm:max-w-2xl md:max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg sm:text-xl font-bold">Results: {selectedItem.item.test_title}</h3>
              <button onClick={handleCloseDetails} className="text-red-600 hover:underline">Close</button>
            </div>
              ...
              <>
                <div className="mt-4 sm:mt-6 grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-center">
                    ...
                    <div className="p-3 sm:p-4 bg-purple-100 rounded-lg">
                        ...
                        <p className="text-lg sm:text-2xl font-bold text-purple-900">{itemDetails.band_score}</p>
                    </div>
                </div>
                ...
                {itemDetails.test_render_structure && Array.isArray(itemDetails.test_render_structure) ? (
                  <div className="space-y-6 sm:space-y-8">
                    {itemDetails.test_render_structure.map((part, partIdx) => (
                      <div key={partIdx} className="border rounded-lg p-3 sm:p-4 bg-gray-50">
                        <h4 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3 text-gray-800">
                          Section {part.part_number}
                          {part.instructions && (
                            <span className="text-xs sm:text-sm font-normal text-gray-600 ml-2">— {part.instructions}</span>
                          )}
                        </h4>
                        <div className="space-y-4 sm:space-y-6">
                          {part.questions.map((q, qIdx) => (
                            <div key={q.id || qIdx} className="mb-4 sm:mb-6">
