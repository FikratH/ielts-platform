...
const Navbar = ({ role, setRole }) => {
  ...
  const adminLinks = (
    <>
      ...
      <Link to="/admin/assignments" className="text-blue-800 font-medium hover:underline underline-offset-4 transition font-sans" onClick={() => setMenuOpen(false)}>Student submissions</Link>
  ...
  return (
    <nav className="bg-white shadow-md border-b sticky top-0 z-50 font-sans">
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-2 sm:py-3 flex justify-between items-center">
        <Link to={!role ? '/login' : (role === 'admin' ? '/admin/assignments' : '/dashboard')} className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="sm:hidden bg-white border-t shadow-md px-4 py-3 flex flex-col gap-3 animate-fade-in-down">
          ...
          {role && (
            <button onClick={handleLogout} className="bg-red-500 hover:bg-red-600 text-white font-bold px-4 py-2 rounded-xl shadow transition font-sans flex items-center gap-2 w-full justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6A2.25 2.25 0 005.25 5.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M18 12l-3-3m3 3l-3 3m3-3H9" /></svg>
              Logout
            </button>
          )}
        </div>
